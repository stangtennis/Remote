...
Agent Version: v3.1.83
Controller Version: v3.1.83
Last Updated: 2026-05-25
Build Server: Ubuntu (192.168.1.92), cross-compile to Windows
Build Script: ./build-local.sh v3.1.83 (builds all 3 exe + NSIS installers)
```

### Downloads
```yaml
Releases: https://github.com/stangtennis/Remote/releases
Agent Installer: https://updates.hawkeye123.dk/RemoteDesktopAgent-v3.1.83-Setup.exe
Agent Console Installer: https://updates.hawkeye123.dk/RemoteDesktopAgentConsole-v3.1.83-Setup.exe
Controller Installer: https://updates.hawkeye123.dk/RemoteDesktopController-v3.1.83-Setup.exe
```
...
### Current Version: v3.1.83 (2026-05-21)
...
1. Check agent version (use a recent `v3.x` release, ideally `v3.1.83+`)
...
### NSIS Installere
```yaml
Agent Setup: https://updates.hawkeye123.dk/RemoteDesktopAgent-v3.1.83-Setup.exe
Agent Console Setup: https://updates.hawkeye123.dk/RemoteDesktopAgentConsole-v3.1.83-Setup.exe
Controller Setup: https://updates.hawkeye123.dk/RemoteDesktopController-v3.1.83-Setup.exe
```
...
CC=x86_64-w64-mingw32-gcc CXX=x86_64-w64-mingw32-g++ \
go build -ldflags '-s -w -H windowsgui -X main.Version=v3.1.83' \
-o ../builds/remote-agent-v3.1.83.exe ./cmd/remote-agent

# Agent Console (with console window for debugging)
cd agent && GOOS=windows GOARCH=amd64 CGO_ENABLED=1 \
CC=x86_64-w64-mingw32-gcc CXX=x86_64-w64-mingw32-g++ \
go build -ldflags '-s -w -X main.Version=v3.1.83' \
-o ../builds/remote-agent-console-v3.1.83.exe ./cmd/remote-agent

# Controller
cd controller && GOOS=windows GOARCH=amd64 CGO_ENABLED=1 \
CC=x86_64-w64-mingw32-gcc \
go build -ldflags '-s -w -H windowsgui -X main.version=v3.1.83' \
-o ../builds/controller-v3.1.83.exe .
```

### Agent Features (v3.1.83)
...
```
Download: https://updates.hawkeye123.dk/RemoteDesktopAgent-v3.1.83-Setup.exe
...
git push && git tag v3.1.83 && git push origin v3.1.83

# Deploy to Caddy (auto-update)
cp builds/remote-agent-v3.1.83.exe ~/caddy/downloads/remote-agent.exe
cp builds/remote-agent-console-v3.1.83.exe ~/caddy/downloads/remote-agent-console.exe
cp builds/controller-v3.1.83.exe ~/caddy/downloads/controller.exe
