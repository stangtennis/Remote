...
// For ASCII characters, uses virtual key codes (VK) instead of KEYEVENTF_UNICODE
// because VK_PACKET (generated by UNICODE mode) doesn't work reliably with
// elevated console windows (PowerShell/CMD running as admin).
func handleUnicode(pipe *pipeRW) error {
	...
	if vk, mods := charToVK(char); vk != 0 {
		...
		if shift {
			inp := makeKeyInput(0x10, 0, 0) // VK_SHIFT down
			inputs = append(inputs, inp[:]...)
		}
		inp := makeKeyInput(vk, 0, 0) // key down
		inputs = append(inputs, inp[:]...)
		inp = makeKeyInput(vk, 0, _KEYEVENTF_KEYUP) // key up
		inputs = append(inputs, inp[:]...)
		if shift {
			inp := makeKeyInput(0x10, 0, _KEYEVENTF_KEYUP) // VK_SHIFT up
			inputs = append(inputs, inp[:]...)
		}
		if altGr {
	...
	// Non-ASCII: use KEYEVENTF_UNICODE (emoji, CJK, accented chars, etc.)
	down := makeKeyInput(0, char, _KEYEVENTF_UNICODE)
	up := makeKeyInput(0, char, _KEYEVENTF_UNICODE|_KEYEVENTF_KEYUP)
...
func charToVK(char uint16) (vk uint16, mods byte) {
	...
	result := int16(ret)
	if result == -1 {
		return 0, 0 // no mapping for this char
	}
