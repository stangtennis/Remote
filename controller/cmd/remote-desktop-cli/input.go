...
func charToKeyCode(c rune) (string, bool) {
	if c >= 'a' && c <= 'z' {
		return fmt.Sprintf("Key%c", c-32), false
	}
	if c >= 'A' && c <= 'Z' {
		return fmt.Sprintf("Key%c", c), true
	}
	if c >= '0' && c <= '9' {
...
func buildTypeText(text string) []string {
	...
	for _, c := range text {
		...
		if code == "" {
			code = "Unidentified" // Use Unicode fallback for unknown chars
		}
		// Include "char" field so agent can use Unicode input (SendInput KEYEVENTF_UNICODE)
		charStr := string(c)
		down := map[string]interface{}{
			"t":     "key",
			"code":  code,
