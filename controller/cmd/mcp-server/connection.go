...
func (cm *ConnectionManager) Connect(deviceID, deviceName string) error {
	...
	log.Printf("[MCP] Connecting to device %s (%s)...", deviceName, deviceID)
	...
	client.SetOnConnected(func() {
		log.Printf("[MCP] WebRTC connected to %s", deviceName)
		conn.mu.Lock()
		conn.connected = true
		conn.mu.Unlock()
		select {
	...
	client.SetOnDisconnected(func() {
		log.Printf("[MCP] WebRTC disconnected from %s", deviceName)
		conn.mu.Lock()
		conn.connected = false
		conn.mu.Unlock()
	})

	// Get ICE servers (with fresh token)
	...
	// Create signaling client (with fresh token)
	...
	select {
	case <-connectedCh:
		log.Printf("[MCP] Connected to %s", deviceName)
...
func (cm *ConnectionManager) GetConnection(deviceID string) (*DeviceConnection, error) {
	...
	if !exists {
		return nil, fmt.Errorf("not connected to device %s — use connect_device first", deviceID)
	}
...
// ListConnections returns all active device IDs
	...
	go func() {
		...
		for range ticker.C {
			...
			for _, id := range toDisconnect {
				log.Printf("[MCP] Idle timeout: disconnecting %s", id)
				cm.Disconnect(id)
			}
		}
	}()
...
func fetchICEServers(supabaseURL, anonKey, authToken string) []webrtc.ICEServer {
	...
	if err == nil && resp.StatusCode == 200 {
		...
		if json.NewDecoder(resp.Body).Decode(&result) == nil {
			...
			if len(servers) > 0 {
				log.Println("[MCP] TURN credentials fetched")
				return servers
			}
		}
	}

	// Fallback: STUN + env TURN
