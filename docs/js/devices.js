...
const AI_SUPPORT_SETUP_SHA256 = '28d27e111faf6c8e72690bdc16edf18bbc6e691e5489994276437111e4fcaea8';
...
const AI_SUPPORT_LEGACY_CLEANUP_SHA256 = '3770d85802ce35cb69580f24752722ca4b6f3c94610e6b7287d8471e3c1aa923';
const AI_SUPPORT_LEGACY_CLEANUP_FALLBACK_URL = 'https://raw.githubusercontent.com/stangtennis/Remote/main/remove-legacy-ai-support-user-windows.ps1';
...
function showAISupportEnrollmentCommand(token, clientName, expiresAt) {
  ...
  const command = `$ProgressPreference = 'SilentlyContinue'; Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass -Force; $dir = Join-Path $env:TEMP 'AISupportSSH'; New-Item -ItemType Directory -Force -Path $dir | Out-Null; $scriptPath = Join-Path $dir 'setup-ai-support-windows.ps1'; $downloadUrls = @('${UPDATES_HOST}/setup-ai-support-windows.ps1?v=${AI_SUPPORT_SETUP_SHA256}', '${AI_SUPPORT_SETUP_FALLBACK_URL}'); $downloaded = $false; $actualHash = ''; foreach ($downloadUrl in $downloadUrls) { try { Remove-Item -LiteralPath $scriptPath -Force -ErrorAction SilentlyContinue; if (Get-Command curl.exe -ErrorAction SilentlyContinue) { & curl.exe --fail --location --silent --show-error --retry 3 --retry-delay 1 --output $scriptPath $downloadUrl; if ($LASTEXITCODE -ne 0) { throw 'download failed' } } else { $webClient = New-Object System.Net.WebClient; try { $webClient.DownloadFile($downloadUrl, $scriptPath) } finally { $webClient.Dispose() } }; $actualHash = (Get-FileHash -LiteralPath $scriptPath -Algorithm SHA256).Hash.ToLowerInvariant(); if ($actualHash -eq '${AI_SUPPORT_SETUP_SHA256}') { $downloaded = $true; break } } catch { } }; if (-not $downloaded) { $size = if (Test-Path -LiteralPath $scriptPath) { (Get-Item -LiteralPath $scriptPath).Length } else { 0 }; throw "AI-support setup-scriptet kunne ikke valideres. Modtaget hash: $actualHash; bytes: $size" }; Unblock-File -LiteralPath $scriptPath; & $scriptPath -EnrollmentUrl ${quote(enrollmentUrl)} -EnrollmentToken ${quote(token)} -SupportPublicKeyUrl ${quote(AI_SUPPORT_PUBLIC_KEY_URL)} -ClientName ${quote(clientName)} -CloudflareAccessHostname 'ssh.hawkeye123.dk'`;
...
async function copyAISupportLegacyCleanupCommand() {
  const quote = (value) => `'${String(value).replace(/'/g, "''")}'`;
  const scriptPath = "Join-Path $dir 'remove-legacy-ai-support-user-windows.ps1'";
  const command = `$ProgressPreference = 'SilentlyContinue'; Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass -Force; $dir = Join-Path $env:TEMP 'AISupportSSH'; New-Item -ItemType Directory -Force -Path $dir | Out-Null; $scriptPath = ${scriptPath}; $downloadUrls = @('${UPDATES_HOST}/remove-legacy-ai-support-user-windows.ps1?v=${AI_SUPPORT_LEGACY_CLEANUP_SHA256}', '${AI_SUPPORT_LEGACY_CLEANUP_FALLBACK_URL}'); $downloaded = $false; $actualHash = ''; foreach ($downloadUrl in $downloadUrls) { try { Remove-Item -LiteralPath $scriptPath -Force -ErrorAction SilentlyContinue; if (Get-Command curl.exe -ErrorAction SilentlyContinue) { & curl.exe --fail --location --silent --show-error --retry 3 --retry-delay 1 --output $scriptPath $downloadUrl; if ($LASTEXITCODE -ne 0) { throw 'download failed' } } else { $webClient = New-Object System.Net.WebClient; try { $webClient.DownloadFile($downloadUrl, $scriptPath) } finally { $webClient.Dispose() } }; $actualHash = (Get-FileHash -LiteralPath $scriptPath -Algorithm SHA256).Hash.ToLowerInvariant(); if ($actualHash -eq '${AI_SUPPORT_LEGACY_CLEANUP_SHA256}') { $downloaded = $true; break } } catch { } }; if (-not $downloaded) { throw "Legacy-oprydningsscriptet kunne ikke valideres. Modtaget hash: $actualHash" }; Unblock-File -LiteralPath $scriptPath; & powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File $scriptPath -Force`;
  try {
    await navigator.clipboard.writeText(command);
    showToast('Legacy-oprydningsscript kopieret.', 'success');
  } catch (_) {
    showToast('Kunne ikke kopiere legacy-oprydningsscriptet.', 'error');
  }
}
