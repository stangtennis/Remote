...
function showAISupportEnrollmentCommand(token, clientName, expiresAt) {
  ...
  const command = `$ProgressPreference = 'SilentlyContinue'; Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass -Force; $dir = Join-Path $env:TEMP 'AISupportSSH'; New-Item -ItemType Directory -Force -Path $dir | Out-Null; $scriptPath = Join-Path $dir 'setup-ai-support-windows.ps1'; $downloadUrl = '${UPDATES_HOST}/setup-ai-support-windows.ps1?v=${AI_SUPPORT_SETUP_SHA256}'; if (Get-Command curl.exe -ErrorAction SilentlyContinue) { & curl.exe --fail --location --silent --show-error --retry 3 --retry-all-errors --output $scriptPath $downloadUrl; if ($LASTEXITCODE -ne 0) { throw 'AI-support setup-scriptet kunne ikke downloades.' } } else { Invoke-WebRequest -UseBasicParsing -Uri $downloadUrl -OutFile $scriptPath }; $actualHash = (Get-FileHash -LiteralPath $scriptPath -Algorithm SHA256).Hash.ToLowerInvariant(); if ($actualHash -ne '${AI_SUPPORT_SETUP_SHA256}') { Remove-Item -LiteralPath $scriptPath -Force -ErrorAction SilentlyContinue; throw 'AI-support setup-scriptet bestod ikke SHA-256 verifikationen.' }; Unblock-File -LiteralPath $scriptPath; & $scriptPath -EnrollmentUrl ${quote(enrollmentUrl)} -EnrollmentToken ${quote(token)} -SupportPublicKeyUrl ${quote(AI_SUPPORT_PUBLIC_KEY_URL)} -ClientName ${quote(clientName)} -CloudflareAccessHostname 'ssh.hawkeye123.dk'`;
