-- AI support sessions are explicitly ended by the administrator. Browser-only
-- screen sharing keeps its existing expiry and cleanup behavior.

UPDATE public.support_sessions
SET expires_at = '9999-12-31T23:59:59.999Z'::timestamptz
WHERE support_mode = 'ai'
...
BEGIN
  ...
  WITH expired_support AS (
    ...
    WHERE status IN ('pending', 'active')
      AND support_mode IS DISTINCT FROM 'ai'
      AND expires_at < now()
    RETURNING id
  )
...
COMMENT ON FUNCTION public.cleanup_old_sessions_direct() IS
  'Cleans remote and browser support sessions while AI support ends explicitly.';
